"use client";
// Panel del coordinador — vista de familias hospedadas y citas del día
...
export default function CoordinadorPage() {
  ...
  const promedioDias = familias.length > 0
    ? Math.round(familias.reduce((acc, f) =>
    ...
    <>
      ...
      <div className="max-w-6xl mx-auto px-4 pt-5 pb-4 md:px-10 md:pt-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          {[
            { icono: Users,    valor: familias.length,  label: "Familias activas",      bg: "#FDF0E6", color: "#C85A2A" },
            { icono: Calendar, valor: citasHoy.length,  label: "Citas hoy",             bg: "#EFF6FF", color: "#3B82F6" },
            { icono: Activity, valor: promedioDias,     label: "Días prom. estancia",   bg: "#FAF5FF", color: "#A855F7" },
            { icono: Clock,    valor: familias.filter(f => citasHoy.some(c => c.familiaId === f.id)).length,
                                                        label: "Familias con cita hoy", bg: "#ECFDF5", color: "#22C55E" },
          ].map(({ icono: Icono, valor, label, bg, color }, i) => (
        ...
        <div className="md:grid md:grid-cols-2 md:gap-8">
          ...
          <section className="mb-6 md:mb-0">
            ...
            {citasHoy.length === 0 ? (
              ...
              <div className="space-y-2">
                {citasHoy.map((cita) => {
                  const fam = familias.find((f) => f.id === cita.familiaId);
                  return (
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-gray-400 text-xs mt-0.5">
                            {format(cita.fecha.toDate(), "HH:mm")} · {fam?.nombreCuidador ?? "Cuidador"}
                            {fam?.nombreNino ? ` — ${fam.nombreNino}` : ""}
                          </p>
                        </div>
          ...
          <section>
            ...
            {familias.length === 0 ? (
              ...
              <div className="space-y-2">
                {familias.map((f) => {
                  const dias = f.fechaIngreso ? differenceInDays(hoy, f.fechaIngreso.toDate()) : 0;
                  return (
                    <div key={f.id} className="bg-white rounded-2xl shadow-sm p-4">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="text-right shrink-0">
                          <span className="text-lg font-bold" style={{ color: "#7A3D1A" }}>{dias}d</span>
