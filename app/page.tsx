  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      /* ──────────────────────────────────────────────
         ...
         ViewBox: 0 0 400 240
         Faro en x≈52, linterna en y≈136
         ────────────────────────────────────────────── */
      const FARO_X = 52
      const FARO_Y = 136
  ...
  return (
    <main className="relative w-full h-screen overflow-hidden bg-[#05091a]">
      ...
      {/* ══════════════════════════════════════════
          ...
          ViewBox 400×240 → se escala a pantalla completa
          ══════════════════════════════════════════ */}
      <svg
        viewBox="0 0 400 240"
      ...
      >
        <defs>
          ...
          {/* Haz principal (núcleo brillante) */}
        ...
        {/* Nebulosa sutil — da profundidad al cielo */}
        ...
        <g ref={starGroupRef}>
          {STARS.map(([xPct, yPct, r, opacity], i) => (
            <circle
              key={i}
              cx={(xPct / 100) * 400}
              cy={(yPct / 100) * 170} // solo en el cielo (0-170)
              r={r}
              fill="white"
              opacity={opacity}
            />
        ...
        {/* ── HAZ DE LUZ (rota alrededor de la linterna) ── */}
        <g ref={beamRef}>
          ...
          {/* Núcleo brillante */}
          <polygon
            points="52,136 490,88  490,184"
            fill="url(#beamCore)"
          />
        </g>

        {/* Reflejo del haz en el océano (sutil, estático) */}
        <polygon
        ...
        {/* ── SILUETAS DE FAMILIAS ──
            Familia = adulto + niño de la mano
            Se iluminan cálidas cuando el haz pasa sobre ellas
        ── */}

        {/* F1 — más cerca del faro (x≈108) */}
        ...
        <g>
          ...
          {/* Franjas horizontales (rayas del faro) */}
      ...
      {/* Gradiente inferior — realza el CTA */}
      <div
      ...
      <div className="relative z-10 h-full flex flex-col">
        ...
        {/* Título + tagline — centrado vertical */}
